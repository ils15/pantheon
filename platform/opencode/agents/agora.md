...
handoffs:
  - label: Document Decision
    agent: mnemosyne
    prompt: Document this architectural decision and its rationale in the Memory Bank as an ADR.
    send: false
  - label: Build the Plan
    agent: athena
    prompt: Create an implementation plan based on the recommendation above.
    send: false
agents:
  - athena
  - apollo
