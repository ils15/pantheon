...
skills: artifact-management, handoff, task-system
user-invocable: true
permission:
  edit: allow
  bash: deny
...
→ Create docs/memory-bank/00-project.md through 00-project.md
→ Create empty 01-active-context.md and 02-progress-log.md
...
→ Update docs/memory-bank/01-active-context.md (new focus, recent decisions, blockers)
→ Append to docs/memory-bank/02-progress-log.md (what was completed, date)
...
**You are NOT invoked automatically after every implementation phase.** Implementation agents may append directly to `01-active-context.md` and `02-progress-log.md`. You are invoked when structure, quality, or explicit documentation is required.
...
├── 00-project.md       ← System design (fill once, update rarely)
├── 00-project.md         ← Component breakdown (update as components change)
├── 00-project.md       ← Tech stack, setup (fill once)
├── 01-active-context.md     ← Sprint focus, decisions, blockers  ← most important
...
**`01-active-context.md` is the priority file.** It is what `copilot-instructions.md` points to. Always keep it current when closing a sprint.

### Session → active context graduation

```
During sprint:   agents write to /memories/session/sprint-plan.md  (ephemeral)
At sprint close: @mnemosyne graduates relevant content to 01-active-context.md (permanent)
```
...
- **`02-progress-log.md` is append-only** — never edit history
...
- **@athena**: Provides plans at sprint start → you graduate them to `01-active-context.md` at sprint close
- **@hermes / @aphrodite / @demeter**: Implement features → they may append to `01-active-context.md` directly; you are invoked for task records
