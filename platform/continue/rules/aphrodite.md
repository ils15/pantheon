...
#### Step 2: Self-Analyze
- Analyze your own screenshot for: layout issues, alignment, spacing, text overflow, contrast, responsiveness
- Use the JSON schema format below to structure your findings
- Be honest — look for issues as if you were a QA tester

#### Step 3: Fix or Escalate
- If issues found: fix them in the component code
- Re-capture screenshot and re-analyze
- After 3 iterations, stop and escalate to Zeus

#### Iteration Tracking
- Count iterations (max 3)
- Each iteration must fix at least one issue
- After 3 iterations, stop and escalate

#### Escalation to Zeus
- Format a structured escalation message
- Include: screenshot path, all iterations, findings, what was fixed, what persists

#### JSON Schema Format
Use this format to structure your analysis:
```json
{
