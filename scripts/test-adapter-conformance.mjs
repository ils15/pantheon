...
const TARGET_PLATFORM = platformArgIndex !== -1 && process.argv[platformArgIndex + 1]
    ...
    if (adapter.frontmatter.include && Array.isArray(adapter.frontmatter.include)) {
      for (const key of adapter.frontmatter.include) {
        ...
        if (canonicalFmKeys.has(key)) {
          pass(platformName, category, `frontmatter.include field "${key}" exists in canonical agents`);
        } else {
          warn(platformName, category, `frontmatter.include field "${key}" not found in canonical agents (may be intentional)`);
        }
      }
    }
    ...
    if (adapter.frontmatter.exclude && Array.isArray(adapter.frontmatter.exclude)) {
      ...
      if (!wildcardExclude) {
        for (const key of adapter.frontmatter.exclude) {
          if (canonicalFmKeys.has(key)) {
            pass(platformName, category, `frontmatter.exclude field "${key}" exists in canonical agents`);
          } else {
            warn(platformName, category, `frontmatter.exclude field "${key}" not found in canonical agents (may be intentional)`);
          }
        }
      } else {
  ...
  if (adapter.excludeTools && Array.isArray(adapter.excludeTools)) {
    ...
    for (const tool of adapter.excludeTools) {
      ...
      if (tool.endsWith('*') && matches.length > 0) {
        pass(platformName, category, `excludeTools pattern "${tool}" matches ${matches.length} known tools`);
        ...
        warn(platformName, category, `excludeTools entry "${tool}" is not a known canonical tool (may be intentional)`);
      }
    }
  }
  ...
  if (adapter.bodyFilters && Array.isArray(adapter.bodyFilters)) {
    for (let i = 0; i < adapter.bodyFilters.length; i++) {
      ...
      if (filter.action === 'omit-section' && !filter.pattern) {
        fail(platformName, category, `bodyFilters[${i}] action "omit-section" missing required "pattern" field`);
      }
      if ((filter.action === 'prepend' || filter.action === 'append') && filter.content === undefined) {
        fail(platformName, category, `bodyFilters[${i}] action "${filter.action}" missing required "content" field`);
      }
    }
  } else {
...
function testAgentOutputConformance(platformName, adapter, canonicalAgentNames) {
  ...
  if (!existsSync(outDir)) {
    for (const name of canonicalAgentNames) {
      fail(platformName, category, `Output directory "${outDir}" does not exist — agent "${name}" cannot be verified`);
    }
    return;
  }
  ...
  for (const name of canonicalAgentNames) {
    ...
    if (exists) {
      ...
      fail(platformName, category, `Missing generated file for agent "${name}" (expected: "${expectedFile}")`);
    }
  }

  // Check for unexpected extra agent-like files
  const generatedAgentNames = new Set(
    generatedFiles.map(f => basenameWithoutExt(f, ext))
  );
  ...
  for (const name of canonicalAgentNames) {
    ...
    if (skipFrontmatter) {
      if (content.startsWith('---\n') || content.startsWith('---\r\n')) {
        fail(platformName, category, `Agent "${name}" has frontmatter (---) but skipFrontmatter is true`);
      } else {
        pass(platformName, category, `Agent "${name}" correctly has no frontmatter wrappers (skipFrontmatter=true)`);
      }
    } else {
      ...
      if (parsed) {
        ...
        for (const key of include) {
          ...
          if (isInGenerated) {
            ...
            fail(platformName, category, `Agent "${name}" is missing frontmatter field "${key}" (present in canonical but not in generated)`);
          } else {
            // Field not in canonical either — this is fine (optional field)
            pass(platformName, category, `Agent "${name}" omits frontmatter field "${key}" (not in canonical agent)`);
          }
        }
        ...
        if (!hasWildcard) {
          for (const key of exclude) {
            ...
            if (parsed.fm[key] !== undefined) {
              fail(platformName, category, `Agent "${name}" has excluded frontmatter field "${key}"`);
            } else {
  ...
  for (const name of canonicalAgentNames) {
    ...
    for (const tool of toolValues) {
      if (mappedPlatformValues.has(tool)) {
        ...
        pass(platformName, category, `Agent "${name}" tool "${tool}" is the ensureAgentTool prepend`);
      } else {
        fail(platformName, category, `Agent "${name}" tool "${tool}" is NOT a valid mapped tool name`);
      }
    }
  }
  ...
  for (const name of canonicalAgentNames) {
    ...
    for (const tool of fmTools) {
      ...
      if (canonicalToolSet.has(tool) && excludeToolsSet.has(tool)) {
        fail(platformName, category, `Agent "${name}" frontmatter contains excluded canonical tool "${tool}"`);
      }
    }
    ...
    for (const ref of toolRefs) {
      ...
      if (excludeToolsSet.has(toolName)) {
        ...
        if (content.includes(`~~${ref}~~`)) {
          pass(platformName, category, `Agent "${name}" excluded tool "${toolName}" properly strikethrough'd`);
        } else {
          warn(platformName, category, `Agent "${name}" excluded tool "${toolName}" NOT strikethrough'd (raw: ${ref})`);
        }
      }
    }
  }
...
function testCapabilityConformance(platformName, adapter) {
  ...
  if (flags.approval === true) {
    ...
    if ('vscode/askQuestions' in toolMap) {
      pass(platformName, category, 'approval is not true but vscode/askQuestions is still mapped (optional enhancement)');
    } else {
...
function testAgentModeConformance(platformName, adapter, canonicalAgentNames) {
  ...
  for (const agentName of Object.keys(overrides)) {
    if (!canonicalAgentNames.includes(agentName)) {
      warn(platformName, category, `agentModeOverrides has entry for non-canonical agent "${agentName}"`);
    }
  }
  ...
  for (const [agentName, mode] of Object.entries(overrides)) {
    if (VALID_MODE_VALUES.has(mode)) {
      ...
      fail(platformName, category, `Agent "${agentName}" has invalid mode "${mode}" (expected primary|subagent)`);
    }
  }
  ...
  if (primaryCount === 0) {
    warn(platformName, category, 'No agents assigned "primary" mode — at least one orchestrator expected');
  } else {
...
function main() {
  ...
  for (const platform of platforms) {
    ...
    console.log(`  ${statusIcon} ${checkCount} checks: ${passCount} pass, ${failCount} fail, ${warnCount} warn`);
  }
