...
export function installOpenCode(
	...
	if (componentSet.has("skills")) {
		...
		if (skillNames.length > 0) {
			...
			const dstSkillsDir = isGlobal
				? join(target, "skills")
				: join(target, ".opencode", "skills");
	...
	if (componentSet.has("commands")) {
		...
		const dstCmds = isGlobal
			? join(target, "commands")
			: join(target, ".opencode", "commands");
		if (existsSync(srcCmds)) {
			const cmdResult = syncDir(srcCmds, dstCmds, dryRun, clean, (f) =>
				f.endsWith(".md"),
			);
	...
	for (const [agentName, agentCfg] of Object.entries(canonicalAgentConfig)) {
		...
		if (config.agent[agentName]) {
			...
			if (!newAgent.permission.bash && agentCfg.permission?.bash) {
				newAgent.permission.bash = JSON.parse(
					JSON.stringify(agentCfg.permission.bash),
				);
			}
