...
function parseArgs(argv) {
  ...
  if (!args.target) {
    ...
    args.target = resolvePath(args.target.startsWith("~") ? args.target.replace("~", homedir()) : args.target);
  }
  ...
  if (args.tier) {
    ...
    if (!validTiers.includes(args.tier)) {
      console.error(`${C.red}${I.error} Invalid --tier value "${args.tier}". Valid: ${validTiers.join(", ")}${C.reset}`);
      process.exit(2);
    }
  }
...
function emit(category, message) {
  counts[category]++;
  if (category === "pass" && !process.argv.includes("--verbose") && !process.argv.includes("--check-only")) return;
  const prefix = I[category] ?? category;
  const color = category === "pass" ? C.green : category === "warn" ? C.yellow : category === "error" ? C.red : C.blue;
  console.log(`  ${color}${prefix} ${message}${C.reset}`);
}

function pass(msg) { emit("pass", msg); }
function warn(msg) { emit("warn", msg); warnings.push(msg); }
function error(msg) { emit("error", msg); }
...
function runSync(args, state) {
  ...
  if (args.dryRun) {
    console.log(`  ${C.blue}${I.info}${C.reset} ${C.dim}Dry-run: would run "npm run sync"${C.reset}`);
...
function runInstall(args, state) {
  ...
  if (args.dryRun) {
    console.log(`  ${C.blue}${I.info}${C.reset} ${C.dim}Dry-run: would run install.mjs --target ${args.target} --platforms ${args.platforms.join(",")}${C.reset}`);
  ...
  try {
    ...
    if (result.status === 0) {
      ...
      const errLines = (stderr || stdout || `exit code ${result.status}`).split("\n").slice(0, 15).join("\n  ");
...
async function verify(target, args) {
  ...
  check(1, "Agent count", () => {
    ...
    if (!existsSync(agentsDir)) return { pass: false, msg: `agents directory not found at ${agentsDir}` };
    const files = readdirSync(agentsDir).filter((f) => f.endsWith(".md"));
    const count = files.length;
    if (count < AGENT_COUNT) {
      return { pass: false, msg: `expected ${AGENT_COUNT} agents, found ${count} (missing: ${AGENT_COUNT - count})` };
    }
  ...
  check(1, "mcp_tools frontmatter", () => {
    ...
    if (missing.length > 0) {
      return { pass: false, msg: `${missing.length} agent(s) missing mcp_tools: ${missing.join(", ")}` };
    }
  ...
  check(1, "Permissions check", () => {
    ...
    for (const file of files) {
      ...
      const hasEditDeny = /edit:\s*deny/.test(fm);
    ...
    if (issues.length > 0) {
      return { pass: issues.length <= 2, msg: `${issues.length} permission issue(s): ${issues.slice(0, 3).join("; ")}` };
    }
  ...
  check(1, "bash=deny agents exclusion", () => {
    ...
    if (issues.length > 0) {
      return { pass: false, msg: `${issues.length} agent(s) have bash=deny but non-empty pantheon-code-mode: ${issues.join("; ")}` };
    }
  ...
  check(2, "MCP Capabilities sections", () => {
    ...
    if (missing.length > 0) {
      ...
      if (missing.length > threshold) {
        return { pass: false, msg: `${missing.length} agent(s) missing MCP Capabilities section (threshold: ${threshold})` };
      }
    }
  ...
  check(2, "Inline compression preserved", () => {
    ...
    for (const file of files) {
      ...
      if (mcpSection && mcpSection[0].includes("See `mcp_tools:`")) {
  ...
  check(3, "Invalid pantheon:// refs", () => {
    ...
    if (invalid.length > 0) {
      return { pass: true, msg: `${invalid.length} unknown ref(s): ${invalid.slice(0, 3).join("; ")}` };
    }
  ...
  const tiers = { 1: { fail: 0, total: 0, label: "Tier 1 (fail-fast)" }, 2: { fail: 0, total: 0, label: "Tier 2 (warn)" }, 3: { fail: 0, total: 0, label: "Tier 3 (info)" } };
  ...
  console.log();
  for (const [num, t] of Object.entries(tiers)) {
  ...
  const tier3Fail = tiers[3].fail > 0;
...
function printSummaryTable(state, verifyResult) {
  ...
  function phaseLine(name, status, detail) {
    const icon = status === "pass" ? `${C.green}${I.pass}${C.reset}` :
                 status === "warn" ? `${C.yellow}${I.warn}${C.reset}` :
                 status === "fail" ? `${C.red}${I.error}${C.reset}` :
                 status === "skipped" ? `${C.blue}${I.info}${C.reset}` :
                 status === "dry-run" ? `${C.blue}${I.info}${C.reset}` :
  ...
  if (state) {
    phaseLine("Sync", state.phases.sync, { pass: "All platforms synced", skipped: "Not run", "dry-run": "Preview mode", fail: "See errors above" }[state.phases.sync] || state.phases.sync);
    phaseLine("Install", state.phases.install, { pass: "All files deployed", skipped: "Not run", "dry-run": "Would install", warn: "Completed with warnings", fail: "See errors above" }[state.phases.install] || state.phases.install);
  }

  if (verifyResult) {
    const verifyStatus = verifyResult.pass ? "pass" : "fail";
    phaseLine("Verify", verifyStatus, `T1:${verifyResult.tier1}/4  T2:${verifyResult.tier2}/4  T3:${verifyResult.tier3}/2`);
  }
...
function mcpAlreadyConfigured(opencodeConfig, mcpKey) {
  return !!(opencodeConfig.mcp && opencodeConfig.mcp[mcpKey]);
}
 ...
 */
function generateMCPEntry(mcpKey, mcp) {
...
function processTier(args) {
  ...
  if (args.tier === "none") {
    ...
    if (tiers.native) {
      ...
      console.log(`  ${C.dim}  ${nativeCount} native MCP(s) always available: ${Object.keys(tiers.native).join(", ")}${C.reset}`);
    }
    console.log();
    return true;
  }
  ...
  for (const [key, entry] of expanded) {
    ...
    const needsConfig = hasNonEmptyConfig(entry.mcp);
    let status;
    if (isNative) {
      status = "native";
  ...
  if (!args.dryRun && args.yes && toAdd.length > 0) {
    ...
    console.log(`  ${C.yellow}${I.info} Use --yes to auto-add ${toAdd.length} pending MCP(s) to opencode.json${C.reset}`);
    console.log();
  }
  ...
  // -------------------------------------------------------------------
  const platformIncludesCopilot = args.platforms.includes("copilot") || args.platforms.includes("all");
  if (platformIncludesCopilot) {
    ...
    if (vscodeResult.generated) {
      ...
      console.log(`  ${C.green}${I.pass}${C.reset} VS Code MCP config: ${mode} ${vscodeResult.path}`);
      console.log(`      ${C.dim}${vscodeResult.servers} server(s) configured${C.reset}`);
      console.log();
    } else {
      const mode = args.dryRun ? "would be" : "is";
      console.log(`  ${C.blue}${I.info}${C.reset} VS Code MCP config: no configurable MCPs for this tier (${mode} skipped)`);
      console.log();
    }
  }
...
async function main() {
  ...
  if (args.checkOnly) {
    ...
    if (!existsSync(join(args.target, "agents"))) {
      console.error(`${C.red}${I.error} No agents directory found at ${args.target} — nothing to verify${C.reset}`);
  ...
  // Determine title
  const title = args.dryRun ? "🔍 Pantheon Install — Dry Run" :
                args.retryFailed ? "🔄 Pantheon Install — Retry Failed" :
                args.clean ? "🧹 Pantheon Install — Clean Install" :
                "✅ Pantheon Install";
  ...
  console.log(`  ${C.dim}Mode:      ${args.dryRun ? "dry-run" : args.clean ? "clean" : args.retryFailed ? "retry-failed" : args.tier ? `tier:${args.tier}` : "full"}${C.reset}`);
  console.log();
  ...
  if (!args.checkOnly) {
    ...
    if (!phaseResult.sync && !args.retryFailed) {
      ...
      console.log(`  ${C.yellow}Tip: Use --retry-failed to resume from last successful phase.${C.reset}`);
  ...
  if (!args.checkOnly) {
    ...
    if (!phaseResult.install && !args.retryFailed) {
      ...
      console.log(`  ${C.yellow}Tip: Fix the issue and use --retry-failed to retry only failed phases.${C.reset}`);
      printSummaryTable(state, null);
      process.exit(2);
    }
  }
