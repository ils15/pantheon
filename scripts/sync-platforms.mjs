...
function loadRoutingYml() {
  ...
  try {
    ...
    const mcpResult = spawnSync('python3', ['scripts/sync-mcp-permissions.py', '--platforms'], {
      stdio: 'inherit',
    })
...
function applyPlatformAgentOverrides(platformName, agentName, fm, body) {
  ...
  if (
    ...
    agentName === 'athena' &&
    typeof nextFm.description === 'string'
  ) {
    nextFm = {
      ...nextFm,
      description: nextFm.description.replace(
        'the user should call @agora.',
        'the user should run /pantheon.',
      ),
    }
  }
 ...
 */
function deploySkills(platformName, adapter, agentFiles, _outDir) {
 ...
 */
function validateBodyToolReferences(
  body,
  agentName,
  platformName,
  finalTools,
  ...
  for (const ref of toolRefs) {
    ...
    console.warn(
      `  ⚠️  [${platformName}/${agentName}] body references tool not available: "${toolName}"`,
    )
    warnings++
  }
  return warnings
...
function transformFrontmatter(fm, adapter) {
  ...
  for (const key of include) {
    ...
    if (
      key === 'mode' &&
      adapter.agentModeOverrides &&
      fm.name &&
      adapter.agentModeOverrides[fm.name]
    ) {
...
function syncPlatform(platformName, adapter, agentFiles) {
  ...
  if (adapter.skipAgentSync) {
    ...
    for (const agentFile of agentFiles) {
      ...
      const finalTools = Array.isArray(finalFm.tools)
        ? finalFm.tools
        : finalFm.permission
          ? Object.keys(finalFm.permission)
          : []
      validateBodyToolReferences(
        finalBody,
        name,
        platformName,
        finalTools,
        toolMap,
        adapter.excludeTools,
      )
  ...
  if (instructionsDeployed > 0) {
    console.log(
      `  📋 ${instructionsDeployed} instructions deployed to ${adapter.instructionsOutputDir}`,
    )
  }
