...
OpenCode uses `provider/model` format for model IDs (e.g., `anthropic/claude-sonnet-4-20250514`). The agents generated by the sync engine retain the VS Code `(copilot)` suffix format, so you may want to override models in `opencode.json`:

```json
{
  "model": "anthropic/claude-sonnet-4-20250514",
  "agent": {
    "apollo": { "model": "anthropic/claude-haiku-4-20250514" },
    "athena": { "model": "anthropic/claude-sonnet-4-20250514" },
    "temis":  { "model": "anthropic/claude-opus-4-20250514" }
  }
}
```

OpenCode also supports **per-phase model profiles** — use cheap models for exploration and powerful models for implementation/review.
