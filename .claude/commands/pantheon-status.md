...
`/pantheon` `/pantheon-status` `/pantheon-status` `/ping` `/audit` `/focus` `/deepwork` `/optimize` `/sketch` `/subtask` `/cancel` `/pantheon-remember` `/pantheon-search` `/pantheon-consolidate` `/pantheon-forget`
