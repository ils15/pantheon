...
## 💰 CUSTO-AWARE DELEGATION

### Subtask-First Rule
**Sempre que possível, use subtask em vez de task completa.** Subtask pula artefato IMPL e revisão Themis, economizando ~40% das chamadas de API por delegação.

### Decision Tree
Antes de cada delegação, execute a árvore de decisão em `src/instructions/zeus-timeout-retry.instructions.md`:

```
□ Scope: ≤2 files AND ≤10 lines?         [YES→continue | NO→full task]
□ Risk: No schema/security impact?        [YES→continue | NO→full task]
□ Auth: No auth logic?                    [YES→continue | NO→full task]
□ Data: No migration/data loss?           [YES→continue | NO→full task]
□ Review: No Themis dependency?           [YES→continue | NO→full task]
ALL YES → subtask | ANY NO → full task
```
